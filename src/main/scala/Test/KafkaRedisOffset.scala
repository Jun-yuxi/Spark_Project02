...
import util.{JedisConnectionPool, JedisOffset, Utils_Time}
...
object KafkaRedisOffset {
  def main(args: Array[String]): Unit = {
    ...
    val brokerList = "192.168.49.161:9092"
    // 编写Kafka的配置参数
    ...
    // 对数据进行处理
    val file = ssc.sparkContext.textFile("C:\\Users\\86178\\Desktop\\Project02\\city.txt")
    ...
    stream.foreachRDD({
      rdd=>
        ...
        // 指标一 1 (统计全网订单数量，充值成功的金额，充值成功的订单数量，充值成功的订单的总时长)
        ...
        //指标 三（每小时一个批次省份订单top10，以及成功率）
        val result4 = baseData.map(t=>((t._3,t._5._1),t._4)).reduceByKey((list1,list2)=>{
          list1.zip(list2).map(t=>t._1+t._2)
        })
